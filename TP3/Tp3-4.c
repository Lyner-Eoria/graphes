...
char *sMOT;
int bTp1AmorceR=kF;
// L4 = a*.am.bn.am.bn, 0<m<n, remonter m et n en S
	// L4' = ap.bn.am.bn, 0<m<n, p>=m
		//L4'' = a+.bn.a+.bn, n>1
// Grammaire algébrique:
			// (a) S(y,z)->K(x).P(y,z).R(x,y,z)
			// (b) K(x)->K1(x)+K2(x)
			// (c) K1(x+1)->A.K(x)
			// (d) K2(1)->A
			// (e) P(m,n)->P1(m,n)+P2(m,n)
			// (f) P1(m,n+1)->B.P(m,n).B
			// (g) P2(m,2)->B.B.K(m).B.B
			// (h) A->a
			// (i) B->b
// Grammaire homogène:
			// (1) S(y,z)->K(x).P(y,z).R(x,y,z)
			// (2) K(x)->Q(x)+U(x)
			// (4) P(m,n)->V(m,n)+W(m,n)
			// (5) Q(x+1)->A.K(x)
			// (6) U(1)->A
