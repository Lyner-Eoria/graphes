...
#define kuDequeCardinalMaX (kP12-1) //nb max de sommets admissible ds la deque; k pour constante;u pour Un ou plus, kP10=2**10=1024). 
...
void DequeTESTER(int iTest){
	...
	Appel0(sC2("DequeTester,test n°",sEnt(iTest)));
		switch (iTest) {
			...
			case 2:
				DequeAllouer(4);
					DequeVoir("Apres DequeAllouer(4)");
					DequeEmpiler(2);
					DequeVoir("Apres DequeEmpiler(2)");
					DequeEmpiler(4);
					DequeVoir("Apres DequeEmpiler(4)");
					DequeEmpiler(3);
					DequeVoir("Apres DequeEmpiler(3)");
					sSource=sDequeDepiler();
					sd("Sommet dépilé",sSource);
					DequeVoir("Apres DequeDepiler()");
					sSource=sDequeDepiler();
					sd("Sommet dépilé",sSource);
					DequeVoir("Apres DequeDepiler()");
				DequeAllouer(0);
			...
			case 3:
				DequeAllouer(4);
					DequeVoir("Apres DequeAllouer(4)");
					DequeEnfiler(2);
					DequeVoir("Apres DequeEnfiler(2)");
					DequeEnfiler(4);
					DequeVoir("Apres DequeEnfiler(4)");
					DequeEnfiler(3);
					DequeVoir("Apres DequeEnfiler(3)");
					sSource=sDequeDefiler();
					sd("Sommet défilé",sSource);
					DequeVoir("Apres DequeDefiler()");
					sSource=sDequeDefiler();
					sd("Sommet défilé",sSource);
					DequeVoir("Apres DequeDefiler()");
				DequeAllouer(0);
		...
		tMode=tModePileOuFilE;
			...
			//afficher la pile stockée dans nPile[]
				...
				for (uS=nCardinal;uS>=1;uS--)
					printf("%2s%s",sEnt(nPile[uS]),(uS>1)?",":".");
				printf(" (%s)\n",sPluriel(nCardinal,"élément"));
