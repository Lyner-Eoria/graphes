...
int nCouplageParfaitListeR(graf *pgG);//O(exponentiel)
...
int nAreteChoisirLenT(graf *pgG,int *psBout0,int *psBout1){//O(S2)
	...
	//sd(sC3b("Arete sélectionnée",sCouple(*psBout0,*psBout1),"pour le cas n°"),nChoisirLent);
	switch (nChoisirLent) {
...
int nAreteChoisirRapidE(graf *pgG,int sOrigine,int *psBout0,int *psBout1){//O(S)
	...
	//sd("nAreteChoisirRapidE:sOrigine",sOrigine);
	...
	//sd(sC3b("Arete sélectionnée",sCouple(*psBout0,*psBout1),"pour le cas"),nChoisirRapide);
...
int nCouplageParfaitListeR(graf *pgG){//O(exponentiel)
	...
	int aK,sX,sY;
	int iCouplage,bCplageParfait,nParfait,nPoids;
	long zAccu,zCas,zCasEnTout;
	int bConfig[1+pgG->nArcEnTout];
	Assert2("nCouplageParfaitListeR",bCouplageAmorceR,nGrapheCycle2(pgG)==0);
	Appel0("nCouplageParfaitListeR");
					...
					for (aK=pgG->aHed[sX];aK<pgG->aHed[sX+1];aK++)
						if (bConfig[aK]){
							...
							if (couplaj[sX]==0 && couplaj[sY]==0){
								couplaj[sY]=-sX;//négatif car origine de l'arc
								couplaj[sX]=+sY;//positif car extrémité de l'arc
								iCouplage++;
							}else iCouplage=kiIntegerMin;
						}
			bCplageParfait=(iCouplage==pgG->nSommetEnTout/2);
			//sd("iCouplage",iCouplage);
			if (bCplageParfait){//afficher le couplage parfait
				//d2(iCouplage,pgG->nSommetEnTout);
				printf("Couplage parfait n°%d:",++nParfait);
				for (nPoids=0,sX=1;sX<=pgG->nSommetEnTout;sX++)
					if (couplaj[sX]){
						printf(" %s",sCouple(sX,abs(couplaj[sX])));
						aK=(couplaj[sX]>0)? nGrapheArc(pgG,sX,couplaj[sX]) : nGrapheArc(pgG,-couplaj[sX],sX);
						Assert1("CouplageParfaitListeR9",aK);
						nPoids+=pgG->nCout[aK];
						//d5(sX,couplaj[sX],aK,pgG->nCout[aK],nPoids);
						couplaj[sX]=couplaj[abs(couplaj[sX])]=0;
					}
				printf(" poids %d.\n",nPoids);
			}
		}// for nParfait
	Appel1("nCouplageParfaitListeR");
	return(nParfait);
}//nCouplageParfaitListeR
	...
	Appel0("CouplageHongroisMaximiseR");
		...
		for (nSommet=2*pgSource->nSommetEnTout,cPasse=0;cPasse<=1;cPasse++){
			if (cPasse>0){//pgG soit initialt un graphe à nSommet sommets et aucun arc
				pgG->nSommetEnTout=nSommet;	//sd("pgG->nSommetEnTout",pgG->nSommetEnTout);
...
int bCouplageOptimiser(graf *pgG,int bPonderer,int bMaximiserSinonMinimiser){
	...
	int sX;
	if (bVoir) Appel0("bCouplageOptimiser");
		...
		if (bVoir) d3(bPonderer,bBiparti,bMaximiserSinonMinimiser);
...
//m2;
	...
	if (bVoir) Appel1("bCouplageOptimiser");
	return(bParfait);
}//bCouplageOptimiser
...
int bCouplageParfaire(graf *pgG,int bVoir){
	...
	if (bVoir) Appel0("bCouplageParfaire");
		Assert1("bCouplageParfaire",bGrapheBiparti(pgG));
		VecteurRazer(bInterface,1,pgG->nSommetEnTout);
		for (int sX=1;sX<=pgG->nSommetEnTout;sX++)
			bInterface[sX]=pgG->aHed[sX]>0;
		GrapheSymetriser(pgG,grCouleur+grCout);
		//GrapheVoir0(pgG,"pgG symétrisé",grCouleur+grCout);
		//d(pgG->nSommetEnTout);
...
int bCouplageParfaiT(graf *pgG,int bVoir){
	...
	if (bVoir) GrapheVoir0(pgG,"pgG symétrisé",grCout);
...
int bCouplagePonderehOptimiseR(graf *pgG,int bMaximum,int bVoir,int bInterface[]){//O(S2A)
	...
	//Appel0("bCouplagePonderehOptimiseR");
		...
		if (bVoir) printf("Recherche d'un couplage de poids %s.\n",(bMaximum)?"maximum":"minimum");
		//GrapheVoir3(pgG,"bCouplagePonderehOptimiseR0");

		GrapheBipartir(pgG,k1Bipartir);//négative pgG->aHed[sX] ssi sX est un sommet de la partition X.
if (bVoir) t( sC3("Élaboration du réseau de transport symétrique à ",sPluriel(pgG->nSommetEnTout+2,"sommet"),"..") );
...
//bVoir=kV;
				if (bVoir)
					GrapheVoir3(pgH,sC2b("Réseau de transport conçu pour acheminer jusqu'à",sPluriel(nFlotRequis,"unité$ de flot")));
			Assert1("bCouplagePonderehOptimiseR45",bGrapheSimple(pgH));
//t("ÉTAP 5");//***************************************************************************************
if (bVoir) t("Calcul du flot optimal..");
			//************li+1, le module Flot a été retiré
			//nFlotObtenu=nFlotMaxCalculer(pgH,sSource,sTrappe,nFlotRequis,0 || bVoir,!k1Preciser);//O(S2A)
			//d4(pgH->nSommetEnTout,pgH->nArcEnTout,nFlotRequis,nFlotObtenu);
			//VecteurVoir("pgH->nPhi",pgH->nPhi,1,pgH->nArcEnTout);
			//ld(yVecteurSomme(pgH->nPhi,1,pgH->nArcEnTout));
if (bVoir) t("Mémorisation du couplage..");
			...
			//couplaj[]:=arcs de pgG qui ont un flot non nul
				...
				if (bVoir) sd("pgG->nSommetEnTout",pgG->nSommetEnTout);
				//d(pgH->nSommetEnTout);
				CouplageSynthetiseR(pgH,!k1Voir);
...
void CouplageSynthetiseR(graf *pgH,int bVoir){//O(A)
	...
	int nCouplage,aK,aL,sX,sY;
	//int sTable[1+pgH->nSommetEnTout][1+pgH->nSommetEnTout];
...
if (bVoir) m2;	//affecter couplaj[] efficact grâce à sTable 
		for (VecteurRazer(couplaj,1,pgH->nSommetEnTout),nCouplage=0,sX=1;sX<=pgH->nSommetEnTout;sX++)//O(A)
			for (aK=pgH->aHed[sX];aK<pgH->aHed[sX+1];aK++){
				sY=pgH->sSuk[aK];
				if (sX<sY && bCroit(1,sX,pgH->nSommetEnTout-2) && bCroit(1,sY,pgH->nSommetEnTout-2)){
					aL=aTablE[sY][sX];
					Assert2("CouplageSynthetiseR2",pgH->nPhi[aK]<=1,pgH->nPhi[aL]<=1);//puisque solution d'un pb de flot unitaire
					if (pgH->nPhi[aK]!=pgH->nPhi[aL]){//car le bilan de deux flux unitaires sur deux arcs tête-bêche est nul
						nCouplage++;
						Assert2("CouplageSynthetiseR2",couplaj[sY]==0,couplaj[sX]==0);
						couplaj[sY]=sX;couplaj[sX]=sY;
						if (0 || bVoir) printf("Couplage: %3d .%d.%d.\n",nCouplage,sX,sY);
					}
				}
			}
if (bVoir) m3;
...
void CouplageTESTER(int iTest){
	...
	int bParfait,nParfait,bMaximum=k1Maximum,bPonderer=k1Poids,bSymetriser=kV;
	graf *pgG;
	...
	Appel0(sC2("CouplageTESTER,test n°",sEnt(iTest)));
		switch (iTest) {
			...
			case  1:	Titrer("kcDomino1");
						GrapheGenerer(kcDomino1,grCouleur,&pgG);
						break;
			case  2:	Titrer("kcDomino2");
						GrapheGenerer(kcDomino2,grCouleur,&pgG);
						break;
			case  3:	Titrer("kcComplet4");
						GrapheGenerer(kcComplet4,grCouleur,&pgG);
						break;
			case  4:	Titrer("kcComplet5");
						GrapheGenerer(kcComplet5,grCouleur,&pgG);
						break;
			case  6:	Titrer("kcFaible");
						GrapheGenerer(kcFaible,grCouleur,&pgG);
						break;
			case  5:	Titrer("kcHerschelFaux");
						GrapheGenerer(kcHerschelFaux,grCouleur,&pgG);
						break;
			case  7:	Titrer("kcHerschel");
						GrapheGenerer(kcHerschel,grCouleur,&pgG);//impair, dc reste 1 sommet non couplé
						break;
			case  8:	Titrer("kcBrexitt");
						GrapheGenerer(kcBrexitt,grCouleur,&pgG);
						break;
			case  9:	Titrer("kcDeltoide");
						GrapheGenerer(kcDeltoide,grCouleur,&pgG);
						break;
			case 10:	Titrer("kcGlobe");
						GrapheGenerer(kcGlobe,grCouleur,&pgG);
						break;
			case 11:	Titrer("kcK221");
						GrapheGenerer(kcK221,grCouleur,&pgG);
						VecteurCopier(pgG->coulh,1,pgG->nArcEnTout,pgG->nCout);
						break;
			case 12:	Titrer("kcK222");
						GrapheGenerer(kcK222,grCouleur,&pgG);
						VecteurCopier(pgG->coulh,1,pgG->nArcEnTout,pgG->nCout);
						break;
			case 13:	Titrer("kcPapillon");
						GrapheGenerer(kcPapillon,grCouleur,&pgG);
						VecteurCopier(pgG->coulh,1,pgG->nArcEnTout,pgG->nCout);
			...
			case 20:	Titrer("kcDomino1");
						...
						GrapheVoir0(pgG,"pgG",grCout);
						nParfait=nCouplageParfaitListeR(pgG);
			...
			case 30:	Titrer("kcLongrine8");
						GrapheGenerer(kcLongrine8,grCouleur,&pgG);
						VecteurCopier(pgG->coulh,1,pgG->nArcEnTout,pgG->nCout);
						GrapheVoir0(pgG,"pgG",grCout);
						nParfait=nCouplageParfaitListeR(pgG);
			...
			case 40:	Titrer("kcLongrine10");
						GrapheGenerer(kcLongrine10,grCouleur,&pgG);
						GraphePonderer0(pgG,1);//tous les arcs
						GraphePonderer(pgG,6,10,grCout,3);//arc (6,10) de poids 3
						GraphePonderer(pgG,9,13,grCout,4);
						GrapheVoir0(pgG,"pgG",grCout);
						nParfait=nCouplageParfaitListeR(pgG);
						Assert1("CouplageTESTER40",nParfait==2);
						break;
			case 50:	Titrer("kcMutex");
						GrapheGenerer(kcMutex,grCouleur,&pgG);
						GraphePonderer0(pgG,1);//tous les arcs
						GrapheVoir0(pgG,"pgG",grCout);
						nParfait=nCouplageParfaitListeR(pgG);
...
void CouplageVoiR2(char *sMessage,graf *pgG,int bMemeSiNul,int bPoids,int nPoidsMaximum){
	...
	int aK,sX,sY;
	int nCouple=0,cPasse,nPoids,nPoidsTotal;
	//Appel0("CouplageVoiR2");
		for (nPoidsTotal=nCouple=0,cPasse=0;cPasse<=1;cPasse++){
			for (sX=1;sX<=pgG->nSommetEnTout;sX++){
				//d(sX);
		...
		printf(". Poids total: %d%s",nPoidsTotal,(bMemeSiNul || nCouple>0)? ".\n": "");
